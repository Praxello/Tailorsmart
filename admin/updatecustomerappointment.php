...
if (isset($_POST['flag']) && isset($_POST['appointmentId']) && isset($_POST['servingEmployeeId']) && isset($_POST['appointmentStatus']) && isset($_POST['appointmentDate'])) {
        ...
        if ($academicQuery1 != null) {
            ...
            if ($academicAffected > 0) {
                ...
                while ($academicResults = mysqli_fetch_assoc($academicQuery1)) {
					...
					if($flag == 1){
						$aptstatus .= ' and has been Rescheduled';
					}
                        ...
                        if ($Allitemdata[$i]['Product'] != null) {
                            if ($Allitemdata[$i]['Fabrics'] != null) {
                                ...
                                for ($j = 1; $j < $selectfabriclen; $j++) {
                                    $html .= '<tr>';
                                    $html .= '<td> </td>';
                                    $html .= '<td style="">'. $Allitemdata[$i]['Fabrics'][$j]['skuNo'].' '. $Allitemdata[$i]['Fabrics'][$j]['fabricTitle'] . '<font color="green"><u>' . $Allitemdata[$i]['Fabrics'][$j]['colorName'] . '</u></font></td>';
                                    $html .= '</tr>';
                                }
                            } else {
                                $html .= '<tr>';
                ...
                $headers .= 'Cc:vikas.pawar@praxello.com,' . "\r\n";
