...
header('Content-Type: application/json');
	...
	$orderItemId = $someArray["customerId"];

	//first delete mapping
	$query = mysqli_query($conn,"delete from customer_measurments where customerId = $orderItemId");
...
//	print($measurmentItems[$key]['value']);
	...
	$measurementValue = $measurmentItems[$key]['value'];
		$query = mysqli_query($conn,"INSERT INTO customer_measurments( `customerId`, `measurmentId`, `value`) VALUES ($orderItemId,$measurementId,'$measurementValue')");
			$rowsAffected=mysqli_affected_rows($conn);
				if($rowsAffected==1)
				{
					...
					//now get mesausrements of this items
												...
												// 	}
												// }
                        // "Data"=>$tempOrderItemMeasurements,
					$response = array('Message'=>"Measurement saved successfully",'Responsecode'=>200);
				}
				else
				{
