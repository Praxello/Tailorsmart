...
header('Content-Type: application/json');
	 ...
	 $academicQuery = mysqli_query($conn,"SELECT * FROM temp_product_master pm inner join product_parent_master parent on pm.parentid = parent.parentid inner join  product_category_master pcm on pm.categoryid = pcm.categoryid inner join product_style_master psm on parent.styleid = psm.StyleId inner join product_substyle_master psm1 on parent.substyleid = psm1.substyleid where pm.isactive=1");
					...
					$response = array('Message'=>"products fetched successfully","Holiday"=>$holidays,"Testimonial"=>$testimonialRecords,"Slots"=>$slots,"Data"=>$records,"Fabrics"=>$fabricrecords,'Responsecode'=>200);	
