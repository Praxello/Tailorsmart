...
$query = mysqli_query($conn,"SELECT scm.serviceid , sm.servicetitle, sm.serviceid, tm.transactionid FROM service_completion_master scm INNER JOIN transaction_master tm ON tm.transactionid = scm.transactionid INNER JOIN service_master sm ON sm.serviceid = scm.serviceid WHERE tm.isactive =1");
if($query!=null) {
...
if($query!=null)
{
	...
	if($academicAffected>0)
	{
		while($academicResults = mysqli_fetch_assoc($query))
		{
			...
			$subscriptionRecords[]=array("CarId"=>$academicResults["carid"],"ServiceId" =>$academicResults["serviceid"],"TransactionId" =>$academicResults["transactionid"], "VehcileNo"=> $academicResults["vehicleno"], "ServiceTitle"=>$academicResults["servicetitle"], "Quota"=>$academicResults["quota"], "Done"=>$remaining);
		}
	}
}

	//all quota records
$query = mysqli_query($conn,"SELECT ccm.vehicleno, tm.purchasetype, tm.total, tm.serviceStartDate, tm.serviceEndDate FROM  transaction_master tm  inner join  customer_car_master ccm on ccm.carid = tm.carid where tm.isactive = 1");
if($query!=null)
{
	$academicAffected=mysqli_num_rows($query);
	if($academicAffected>0)
